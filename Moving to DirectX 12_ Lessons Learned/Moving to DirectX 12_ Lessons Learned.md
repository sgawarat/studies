...
- シェーダプロファイル5.1に伴う問題に用心する(レジスタスペースを使いたい場合)。
    - このプロファイルではFXCは若干不安定(だったが、今では多くの問題が直っている)。
    - さまざまなシェーダ最適化制約(fxcのフラグ"/all_resources_bound"[@Ruiz2016]を参照)。
...
- パフォーマンスについて考えたり、最適なシェーダを生成したりするとき、事前コンパイルされたレンダステート+シェーダは素晴らしい。
    - 実践では、殆どのエンジンはかなり緩やかなアプローチを取っており、アーティスト向けインターフェイスによるレンダステートの細かな変更すら許可していた。[?]
