...
- $\forall_x \in \mathbf{x}, 0 \ge P(x) \ge 1$である。不可能な事象は確率0を持ち、状態はそれより小さな確率を取り得ない。同様に、起こることが保証される事象は確率1を持ち、状態はそれ以上の発生確率を取り得ない。
...
$$
\sum_i P(\mathbf{x} = x_i) _ \sum_i \frac{1}{k} = \frac{k}{k} = 1
$$
