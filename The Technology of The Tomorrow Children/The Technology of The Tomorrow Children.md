...
- GPU Pro 3のスクリーンスペースベント(bent)コーンと似たようなことをやっている。
- スカラの遮蔽値ではなく2バンドのSH(球面調和関数)を統合する。
    - 特にスペキュラを扱うときに役に立つ。
- 可視性コーンに簡単に変換できる。
- 各トレース方向のコーンと可視性コーンを交差して、適宜入射光を調整する。

# キャラクター(Characters)

- キャラクターはそのサイズとオーバーヘッドによりボクセル化していない。
- コーンによる遮蔽テストの代わりに、コリジョンカプセルを使う。
- これは"Lighting Technology of The Last of Us"の方法と良く似ている。
    - ただし、主方向ではなく16方向で行う。
    - 16レイヤのディープスクリーンスペーステクスチャ配列を得る。

# 乗り物(Vehicles)

- このゲームで乗り物と呼ばれる動的オブジェクトは、カプセルにうまくフィットしない。
- 可視性を2バンドのSHに統合し、3Dテクスチャに格納する。
    - 16方向のコーンとの交差が簡単になる。
    - キャラクターと同じく、16レイヤのディープテクスチャ配列に出力される。

# パーティクル(Particles)

- パーティクルは潜在的にそのピクセルごとに16方向のコーントレースが必要になる。
    - コストがかかりすぎる。
- カスケードテクスチャをシンプルな放射照度プロブのような2バンドのSHに簡略化したもの使う。
    - テクスチャルックアップは3回で済む。
- パーティクルをテッセレートして、頂点ごとにテクスチャからサンプルする。
- 遮蔽情報はコーントレースにフィードバックされる。
    - 動的な遮蔽テクスチャに格納する。

# 表面下散乱(Subsurface Scattering)

- SS的な見た目を得るには、マテリアル内部のバウンスをシミュレートする必要がある。
- 可能な方法としては、テクスチャ空間かジオメトリ対応のスクリーン空間でライティング情報をぼかす。
    - オブジェクトの背後にあるライトがにじみ出てくる現象を扱えなくなる。

- パーティクルのために作ったSHテクスチャが役立つ。
    - このテクスチャは事実上その空間の任意の地点をサンプルできるライトの場(field)である。
    - つまりはその地点の放射輝度を得ることができる。
- カスケードレベルからはより大きな空間の入射光情報を得られる。
- 様々なカスケードレベルから放射照度の重み付き平均を取り、視点には直接的にヒットしていないライトを収集するのに用いることができる。
- 我々の実装では、このライティングを累計するため、第1SHバンドを取ることで球全体をサンプルする。
    - 我々はこれを"static SS"シェーディングと呼ぶ。

- 視点から離れるようにオブジェクトを通ってレイマーチングしたり、様々なカスケードレベルからサンプリングしたり、レイマーチの方向に射影したSHコーンを使ってライトを集めたりすることで指向性を持つ効果を得ることもできる。
    - 我々はこれを"directional SS"と呼ぶ。
- マテリアルルックのリッチな範囲を得るため、これら2つの表面下のライティング結果とコーントレーシングで得られる通常のディフューズライティングとの間を補間する。
- マテリアルのアルベドをいじって見た目を強調するために"frosting"と呼ぶパラメータを導入する。
    - SSDOと一緒に計算されるスクリーンスペースの厚さパラメータを使う。

# 符号付き距離場(Signed Distance Field)

- Jump Floodingによりカスケードのための3DのSigned Distance Fieldを高速に構築できる。
- ランドスケープとオブジェクト用にひとつ、動的ライト用にひとつ。
- これがあれば、ボクセルデータを通じてレイマーチを高速化できる。

# レイマーチによる反射(Ray Marched Reflections)

- 表面やライトに接近したとき、SHのカスケードテクスチャからサンプルして、結果を蓄積できる。
- 非常に粗いが、光沢のある(glossy)スペキュラ反射には十分。
- スクリーンスペースに依存しないので、画面外のオブジェクトも反射できる。

# 屈折(Refractions)

- このアプローチは光沢のある(glossy)屈折を表すように拡張することもできる。
