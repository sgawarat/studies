...
## Microfacet[^microfacet]モデル
![][fig-microfacet]

Microfacetモデルは，マクロスケールにおける平らな面がミクロスケールにおける粗い面で構成されていると仮定する．物体表面の領域$dA$の陰影は，microfacet同士の遮蔽関係を考慮しながら入射光ベクトル$l$を視点ベクトル$v$に反射するmicrofacetの分布により計算する．このとき注目するmicrofacetの法線は入射光ベクトル$l$と視点ベクトル$v$の中間に当たる．これは**half-vector**と呼ばれ，以下で定義される．

$$
{\bf h} = \frac{{\bf l} + {\bf v}}{|{\bf l} + {\bf v}|}
$$

そして，等方性の(isotropic)材質におけるmicrofacetモデルの一般的な式は以下の通り．

$$
f(l,v) = {\rm diffuse} + \frac{D(\theta_h)F(\theta_d)G(\theta_l, \theta_v)}{4\cos\theta_l \cos\theta_v}
$$

ここで，$\theta_h$は法線とhalf-vectorのなす角を，$\theta_d$は$l$とhalf-vectorのなす角を，$\theta_l$，$\theta_v$は$l$，$v$それぞれと法線のなす角を表す．

diffuse項は決まった形はなく，よく使われるLambert diffuseでは定数になる．

specular項は3つの要素に分けられる．

- $D(\theta_h)$はNormal distribution function; NDF[^NDF]と呼ばれる．どれだけのmicrofacetがhalf-vector方向を向いているかを表し，スペキュラのピーク形状に影響する．
- $F(\theta_d)$はFresnel反射を表す．
- $G(\theta_l, \theta_v)$は入射光がmicrofacetで反射して視点に届くまでの間に他のmicrofacetに遮蔽されない確率を表す．
