...
1. ステンシルバッファを0クリアする
2. 最も遠いライトの境界(boundary)の手前にあるピクセルに印をつける
3. ライトボリュームの内側にある照らされているピクセル(lit pixels)を数える
4. 照らされているピクセルの陰影付けを行う

#### ピクセルの印付け

ライトボリュームの背面より手前にあるピクセルに対して、対応するステンシルバッファの値を1にする。
このときのパイプラインステートは以下の構成に従う。
...
印を付けられたピクセルのうち、ライトボリュームの前面より奥にあれば、そのピクセルはライトの影響範囲内にあることが分かる。
このときのパイプラインステートは以下の構成に従う。
...
この構成でライトボリュームを表現したジオメトリをレンダリングすると、陰影付けを行うべきピクセル数がオクルージョンクエリにより取得できる。この数をしきい値にかけることで、シェーディング処理を省略することもできる。
