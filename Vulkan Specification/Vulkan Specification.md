...
デスクリプタはデスクリプタセットオブジェクトに一緒にグループ化される。デスクリプタセットオブジェクトはそのタイプとデスクリプタ数がデスクリプタセットレイアウトによって定義されるデスクリプタ一式のためのストレージを含む不透明オブジェクトである。レイアウトオブジェクトはメモリやその他のハードウェアリソースと各デスクリプタバインディングの関連を定義するのに使用**してもよい**。レイアウトはデスクリプタセットと関連する必要があるリソースを決定するため、および、シェーダステージとシェーダリソースの間のインターフェイスを決定するため、の両方に使われる。

### デスクリプタセットレイアウト {id="sec:13.2.1"}

デスクリプタセットオブジェクトは0個以上のデスクリプタバインディングの配列によって定義される。それぞれのデスクリプタバインディングはデスクリプタタイプ、バインディング内のデスクリプタ数の個数(配列のサイズ)、バインディングにアクセス**できる**シェーダステージのセット、(イミュータブルサンプラを使うならば)サンプラデスクリプタの配列によって指定される。

TODO: `VkDescriptorSetLayout`

`VkDescriptorSetLayoutBinding`構造体は以下のように定義される。
...
- `binding`はこのエントリのバインディング番号であり、シェーダステージにおける同じバインディング番号のリソースに対応する。
- `descriptorType`はこのバインディングで使われるリソースデスクリプタタイプを指定する[VkDescriptorType](#)である。
- `descriptorCount`は配列としてシェーダでアクセスされる、バインディングに含まれるデスクリプタ数である。`descriptorCount`が0ならば、バインディングエントリは予約され、そのリソースはそのセットレイアウトを使うパイプライン内でこのバインディングを介してステージからアクセス**してはならない**。
- `stageFlags`メンバーはこのバインディングでリソースにアクセス**できる**パイプラインシェーダステージを指定する[VkShaderStageFlagBits](#)のビットマスクである。`VK_SHADER_STAGE_ALL`は、拡張によって定義される追加のステージを含めた定義されるすべてのシェーダステージがリソースにアクセス**できる**ことを指定する簡略記法である。
  <!--  -->
  あるシェーダステージが`stageFlags`に含まれない場合、リソースはセットレイアウトを用いるパイプライン内でこのバインディングを介してそのステージからアクセスされ**てはならない**。フラグメントシェーダに制限される入力アタッチメント以外では、ステージの組み合わせがデスクリプタバインディングによって使用**できる**、特に、バインディングがグラフィクスステージとコンピュートステージの両方で使用**できる**ことでの制限はない。
- `pImmutableSamplers`はサンプラの初期化に影響を与える。`descriptorType`が`VK_DESCRIPTER_TYPE_SAMPLER`か`VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER`を指定するならば、`pImmutableSamplers`は*イミュータブルサンプラ[immutable samplers]*のセットを初期化するのに使用**できる**。イミュータブルサンプラはそのセットレイアウトに永続的にバインドされ、後にデスクリプタセットにおけるイミュータブルサンプラスロットにサンプラをバインドすることは許可されない。`pImmutableSamplers`が`NULL`でないならば、セットレイアウトによって消費され、対応するバインディングのために使われるであろうサンプラハンドルの配列へのポインタと見なされる。`pImmutableSamplers`が`NULL`であるならば、サンプラスロットは動的であり、サンプラハンドルはこのレイアウトを用いるデスクリプタセットにバインドされ**なければならない**。`descriptorType`がこれらのデスクリプタタイプのひとつでないならば、`pImmutableSamplers`は無視される。

上記のレイアウトの定義はデスクリプタバインディングが、0から最大バインディング番号までのすべてのバインディング番号を`pBindings`配列に指定する必要がないように、まばらに指定できるようにする。指定されないバインディングは0の`descriptorCount`と`stageFlags`を持ち、`descriptorType`は未定義として扱われる。しかし、`pBindings`配列における0から最大バインディング番号までのすべてのバインディング番号は、すべてのデスクリプタバインディングが使われていなくても、デスクリプタセットレイアウトにおいてメモリを消費**してもよい**が、デスクリプタプールから追加のメモリを消費**しないほうがよい**。

!!! note
    指定される最大バインディング番号は無駄なメモリを回避するために可能な限りコンパクトにする**必要がある**。

TODO: Valid Usage

### パイプラインレイアウト {id="sec:13.2.2"}

パイプラインからデスクリプタセットへのアクセスは*パイプラインレイアウト[pipeline layout]*を通して達成される。0個以上デスクリプタセットレイアウトと0個以上のpush constant範囲はパイプラインによってアクセス**できる**完璧なリソースセットを記述するパイプラインレイアウトオブジェクトを形成するために組み合わせられる。パイプラインレイアウトは固有のレイアウトを持つ各々を伴うデスクリプタセットの列を表す。このレイアウトの列はシェーダステージとシェーダリソースとの間のインターフェイスを決定するのに使われる。各パイプラインはパイプラインレイアウトを用いて生成される。

TODO: `vkCreatePipelineLayout`

ひとたび生成されれば、レイアウトはパイプライン生成の一部として([パイプライン](#)を参照)、デスクリプタセットのバインディングの一部として([デスクリプタセットバインディング](#)を参照)、push constantsの設定の一部として([Push Constant更新](#)を参照)、用いられる。パイプライン生成は入力としてパイプラインレイアウトを受け入れ、レイアウトはデスクリプタセット内のハードウェアリソースかメモリロケーションへの(セット, バインディング, 配列要素)のタプルに対応するために用いられ**てもよい**。ハードウェアリソースの割り当てはパイプラインレイアウトを構成するデスクリプタセットに定義されるバインディングにのみ依存し、シェーダソースには依存しない。

パイプラインにおけるすべてのシェーダで[静的に用いられる](#)すべてのリソース変数は対応するデスクリプタセットレイアウトに存在し、適切なデスクリプタタイプであり、使われるシェーダステージを`stageFlags`に含む(セット, バインディング, 配列要素)で宣言され**なければならない**。パイプラインレイアウトは特定のパイプラインによって使われない、または、いずれのシェーダからも消去されたデッドコードであるエントリを含めることが**できる**。パイプラインレイアウトはアプリケーションが複数のパイプラインコンパイルにわたって矛盾のないバインディングセットを提供できるようにする。これは、これらのパイプラインが、実装がバインディングをリプログラミングせずにパイプラインを安価に切り替え**てもよい**方法で、コンパイルされることを可能にする。

同様に、各シェーダに宣言されるpush constantブロックは(表すならば)使われるシェーダステージに対応するビットを含む`stageFlags`を持つpush constant範囲にそれぞれ含まれるオフセットにある変数のみを配置**しなければならない**。パイプラインレイアウトは特定のパイプラインによって使われない、または、いずれのシェーダからも消去されたデッドコードとなった変数のための範囲や範囲の部分を含むことが**できる**。

[パイプラインレイアウトリソース制限](#)に示されるようにパイプラインレイアウトにおけるすべてのデスクリプタセットレイアウトでのバインディングに含めることが**できる**各タイプのリソースの総数の制限が存在する。"利用可能なリソースの総数"の列はパイプラインレイアウトにおけるすべてのデスクリプタセットレイアウトでのバインディングに含めることが**できる**リソースの各タイプの総数の制限を与える。加えて、[シェーダリソース制限](#)に述べられるようにパイプラインステージで使われることが**できる**リソースの各タイプの総数の制限が存在する。

TODO: Table 10. Pipeline Layout Resource Limits

TODO: `vkDestroyPipelineLayout`

#### パイプラインレイアウト互換性

2つのパイプラインレイアウトは、同一のpush constant範囲で生成された場合、"[push constants](#)に対して互換性がある"と定義される。2つのパイプラインレイアウトは、0からNまでのセットに対する*同一に定義された[identically defined]*デスクリプタセットレイアウトで生成され、同一のpush constants範囲で生成された場合、"セットNに対して互換性がある"と定義される。

セット番号Nへデスクリプタセット([デスクリプタセットバインディング](#)を参照)をバインディングするとき、0からN-1までのセットに対して以前にバインドされたデスクリプタセットがすべて互換性のあるパイプラインレイアウトを用いてバインドされた場合、このバインディングの処理はそれより番号の小さいセットを妨げない。加えて、セットNに対して以前にバインドされたデスクリプタセットがセットNに対して互換性のあるパイプラインレイアウトを用いてバインドされた場合、Nより大きい番号のセットにおけるバインディングもまた妨げられない。

同様に、パイプラインをバインディングするとき、それより番号の小さいセットが互換性のあるレイアウトでバインドされている限り、パイプラインは互換性のあるパイプラインレイアウトでバインドされた以前にバインドされたデスクリプタセットに正確にアクセス**できる**。

レイアウト互換性は、特定のパイプラインを最初にバインドしなくても、デスクリプタセットが互換性のあるパイプラインレイアウトで生成されたパイプラインによってコマンドバッファにバインドさせることが**できる**ことを意味する。それは、デスクリプタセットがパイプラインの変更をまたいで有効のままであることが**でき**、同じリソースが新しくバインドされたパイプからアクセス可能であろうことも意味する。

TODO: Implementor's Note

!!! note
    変更頻度の低いデスクリプタセットをパイプラインレイアウトの最初の方に配置し、変更頻度の高いデスクリプタセットは最後の方に配置すること。パイプラインが切り替えられるとき、無効化されたデスクリプタセットバインディングのみが更新される必要があり、デスクリプタセットバインディングの残りはそのままになるだろう。

パイプラインレイアウトへバインドされることが**できる**デスクリプタセットの最大数は物理デバイス特性からクエリされる([制限](#)の`maxBoundDescriptorSets`を参照)。
