...
|Xbox360の並び替え|同期なし|lwsync|sync|
|-|-|-|-|
|読み込みが読み込みの先に移動する|Y|N|N|
|書き込みが書き込みの先に移動する|Y|N|N|
|書き込みが読み込みの先に移動する|Y|N|N|
|読み込みが書き込みの先に移動する|Y|Y|N|
...
- キャッシュ
    - キャッシュはキャッシュコヒーレンシーを保証するためにメインメモリにいかなるデータも書き込む必要は**ない**。
    - 他の最適化として、本当に必要とされるまでメインメモリへの書き込みを更に抑制する方法が存在する(例: MOESI)。
- メインメモリ
    - I/Oデバイス(読み込みGPU)はキャッシュではなくこれを見ている。あなたは、他のデバイスが読めるようにしたい場合、データをメインメモリへきちんと強制的に書き込む必要がある。
    - メインメモリは本当に遠く離れている。

# コンペア・アンド・スワップ(CAS)[Compare And Swap (CAS)]

- 整列[aligned]したネイティブサイズのメモリロケーションのアトミック更新
...
- PowerPC/PS3:
    ...
    - **ほかのすべての先立つロード/ストアが他のコアから見えていることを保証しているわけではない。**
        - **解: CAS成功後に`__lwsync`か`__isync`を追加する。**
- Intel/AMD
...
|-|-|-|
|lwsync|33-48|20-90|
|InterlockedIncrement CAS(OS関数)|225-260|36-90|
|クリティカルセクション(Acq+Rel)|~345|40-100|
|ミューテックス(Acq+Rel)|~2350|750-2500|

# スレッドセーフ != コンカレント(並行)[Thread-Safe != Concurrent]

- スレッドセーフ(私自身の定義)
    - 多数のスレッドがこの関数を安全に呼び出すことができる。
    - ひとつより多いスレッドでは事が進むこと[progress]を保証しない。^[訳注:処理が競合する所では他方をブロックすることがある、ということ？]
- コンカレント(私自身の定義)
    - 多数のスレッドがこの関数を安全に呼び出すことができる。
    - ほとんどのスレッドはいつでも事が前に進んでいる[are having forward progress]。^[訳注:ブロックしない設計になっている、ということ？]

# ベストプラクティス[Best Practice]

- OSが提供するクリティカルセクションを用いることを推奨する。
- 取得/解放メモリバリアを用いる。
- 共有するデータ/フラグを128バイト境界に整列[align]する。
    - ハードウェアプリフェッチによりIntel/AMDでも。
- PS3: 疑わしいときや、ロック後にデータを読み込むときは、`lwsync`より前[over]に`isync`を使う。
- チェックインする**前**にコードが機能していることを保証するため、コードがこのレベルで書かれたいかなるコードも同僚たちにレビューさせる。

# おみやげ[Take Away]

- 他のレイヤーへのデータを矯正するための固有の命令を使う。
    - キャッシュレイヤー(他のコア) --- `lwsync`と`isync`
    - メインメモリ(IOデバイス) --- eioioと`sync`
- SPUプログラミングやDMAのやつに少し似ている。
    - レイヤー間でデータを明示的に'転送'する。
- これはプロセッサが動作の仕方の非常に単純化した見方である。
    - 例: メモリアクセスモード
        - Write-Through
        - Cache-Inhibited
        - Cache-Coherent
        - Guarded
- 恐れよ！！[Be scared!!]
    - 恐ろしくないとしたら、このプレゼンテーションを理解しなかったということだ。
