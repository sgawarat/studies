...
解決前に適用しようとする場合、MLAA以上の改善を見せるが、ブラーが強すぎて依然としてあまり良くない。

<!-- p.75 -->

逐一説明しないが、サブピクセル位置を合わせるために再ベクトル化をオフセットすることで、どうにかしてさらに良い結果を得る。
...
それと、MLAAによって導入される一般的な粗さを回避することを可能にする鮮鋭なジオメトリ特徴も達成する。

たった2つの追加のメモリアクセスのコストで、より良いテキストハンドリングを有効化する。

<!-- p.80 -->

- 正確な検索

我々の単純化した検索スキームは、とても高速である一方、ディザリングの形でアーティファクトを引き起こす。

これは、ピクセル間の中間でサンプルするため、最終段が曖昧になる、という事実によるものである。
...
我々が導入したもうひとつの機能は局所的コントラスト認識[local contrast awareness]と呼ぶものである。

MLAAアプローチの大きな欠点は通常エッジをオンかオフの二値と見なすことである。
...
だから、我々が行ったことは、我々のビジュアルシステムを真似して、近傍について低コントラストを持つエッジを無視することである。
...
我々のテクニックの重要な要素は、どれがピクセルに対する最適なパターンの再ベクトル化かを決定するために用いる経験則であり、長きに渡りそれを続けることであると思う。
...
このテクニックはモジュール式であり、利用可能な予算に合わせて容易に、任意に機能をオンオフできる。
...
そして最後に、高品質プロファイルのパフォーマンスは720pバッファひとつに対して1msで動作する。
